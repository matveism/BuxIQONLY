...
import { DiscordLogin } from '@react-oauth/discord';
...
const Index = () => {
  ...
  const handleGoogleLoginSuccess = (credentialResponse: any) => {
    ...
    // Simulate login
    login('google-user', 'google-id', 'bypass-captcha');
  };
  ...
  const handleDiscordLoginSuccess = (credentialResponse: any) => {
    const decoded = jwtDecode(credentialResponse.credential || '');
    console.log('Discord login success:', decoded);
    toast.success('Discord login successful!');
    // Simulate login
    login('discord-user', 'discord-id', 'bypass-captcha');
  };

  const handleDiscordLoginError = () => {
  ...
  const openSupportTicket = () => {
    ...
    // In a real app, you would send this to your backend/Google Sheets
    // createSupportTicket(ticketMessage);
  };
  ...
  return (
    <div className="min-h-screen bg-background">
                  ...
                  {isSignUp ? (
                    <form onSubmit={handleSignUp} className="space-y-4">
                      ...
                      <div className="flex justify-center gap-4">
                        ...
                        />
                        <DiscordLogin 
                          onSuccess={handleDiscordLoginSuccess}
                          onError={handleDiscordLoginError}
                        />
                      </div>
            ...
            {isLoggedIn ? (
              <div className="max-w-2xl mx-auto bg-card p-6 rounded-xl shadow-lg border border-border">
                ...
                <div className="mt-4 space-y-2">
                  <p>
                    <strong>Account Number:</strong> {currentUser.account}
                  </p>
                  <p>
                    <strong>Client ID:</strong> {currentUser.clientId}
                  </p>
                  <p>
                    <strong>Balance:</strong> {Math.floor(userBalance)} pts
                  </p>
                  <p>
                    <strong>Level:</strong> {userLevel}
                  </p>
                  {/* Additional profile info and settings */}
                </div>
              ...
              <div className="max-w-2xl mx-auto bg-card p-6 rounded-xl shadow-lg border border-border">
                ...
                {/* Optionally, add login form here or redirect to login */}
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'help' && (
          <div className="space-y-8 animate-fade-in">
            <div className="max-w-2xl mx-auto bg-card p-6 rounded-xl shadow-lg border border-border">
              ...
              <p className="mb-4">
                Need assistance? Create a support ticket below or contact us via chat.
              </p>
              <Button className="w-full" onClick={openSupportTicket}>
                Create Support Ticket
              </Button>
            </div>
            {/* Optionally, include live chat or FAQ */}
          </div>
        )}
      </main>

      {/* Chat Modal or Sidebar */}
      <div className="fixed bottom-0 right-0 m-4 w-96 bg-background border border-border rounded-lg shadow-lg z-50">
        <div className="flex items-center justify-between p-3 border-b border-border">
          <h3 className="font-semibold">Support Chat</h3>
          <button onClick={() => setIsChatOpen(!isChatOpen)} aria-label="Toggle Chat">
            <X className="w-4 h-4" />
          </button>
        </div>
        {isChatOpen && (
          <div className="flex flex-col h-96 p-2 overflow-y-auto">
            <div className="flex-1 space-y-2 mb-2">
              {chatMessages.map((msg, idx) => (
                <div key={idx} className={`flex flex-col ${msg.sender === (currentUser?.username || 'You') ? 'items-end' : 'items-start'}`}>
                  <div className={`p-2 rounded-lg ${msg.sender === (currentUser?.username || 'You') ? 'bg-primary text-primary-foreground' : 'bg-muted'} max-w-xs`}>
      ...
      {/* Additional components like ActivityLogs, Notifications, etc., can be added here */}
    </div>
  );
};
